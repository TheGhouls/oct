...
class Application:
    def __init__(self, root, hosts):
        ...
        self.text_box = tkinter.scrolledtext.ScrolledText(self.root, width=59, height=24, font=('Helvetica', 9))
    ...
    def run_tests(self):
        self.clear_window()
        for host, port in self.hosts:
            server = xmlrpc.client.ServerProxy('http://%s:%s' % (host, port))
            try:
    ...
    def update_configs(self):
        self.clear_window()
        f = tkinter.filedialog.askopenfile(parent=self.root, initialdir='./', title='Select a Config File')
        for host, port in self.hosts:
            server = xmlrpc.client.ServerProxy('http://%s:%s' % (host, port))
            try:
