...
def resp_graph(avg_resptime_points_dict, percentile_80_resptime_points_dict,
    ...
    fig = pygal.Line(human_readable=True, x_title='Elapsed Time In Test (secs)', y_title='Response Time (secs)')
    x_seq = sorted(avg_resptime_points_dict.keys())
    fig.y_scale = 0.5
    fig.add('AVG', [avg_resptime_points_dict[x] for x in x_seq])
    x_seq = sorted(percentile_90_resptime_points_dict.keys())
    fig.add('90pct', [percentile_90_resptime_points_dict[x] for x in x_seq])
    x_seq = sorted(percentile_80_resptime_points_dict.keys())
    fig.add('80pct', [percentile_80_resptime_points_dict[x] for x in x_seq])
    fig.x_labels = map(str, range(1, len(x_seq) + 1))
    fig.render_to_file(filename=dir + image_name)
